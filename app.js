...
async function loadModel(){
  if(typeof tf==='undefined' || typeof mobilenet==='undefined'){
    $('status').textContent='libs blocked';
    banner('err','TensorFlow libraries did not load (network blocked them). The app UI still works, but detection needs them. Try again on open wifi/data for the first load.');
    return;
  }
  try{
    ...
    tf.tidy(()=>model.infer(tf.zeros([1,224,224,3]), true)); // warmup
    $('status').textContent = 'model ready';
  }catch(e){
    $('status').textContent = 'model failed';
    banner('err','Model failed to load. Needs internet on first run (it caches after). UI still works.');
    console.error(e);
  }
}
...
window.addEventListener('error', ev=>{
  const s=$('status'); if(s) s.textContent='JS error (see console)';
  console.error('DefectCam error:', ev.message, ev.filename, ev.lineno);
});
try{ load(); refreshCounts(); bindPaint(); }catch(e){ console.error(e); }
loadModel();
